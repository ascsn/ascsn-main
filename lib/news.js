...
export function getAllNewsIds() {
  ...
  return fileNames.map(fileName => {
    const id = fileName.replace(/\.md$/, '')
    // Optional: Create a more SEO-friendly ID if needed, like in posts.js
    // const seoFriendlyId = id.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase()
    return {
      params: {
        id: id // Using the direct filename as ID for now
      }
    }
  })
}
...
export async function getNewsData(id) {
  ...
  return {
    id,
    contentHtml,
    // Optional: Add metaDescription if news items will have descriptions in frontmatter
